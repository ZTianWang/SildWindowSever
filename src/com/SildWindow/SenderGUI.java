...
//		List senderL,timeL,dateTypeL,stateL,typeL,winNumL;
		...
		paraSet.add(new Label("发送窗口："));
		paraSet.add(sendWinNum);
		paraSet.add(new Label("发送定时器："));
		paraSet.add(sendTime);
		paraSet.add(new Label("重发定时器："));
		...
		errorSet.add(new Label("出错数据帧（空格分隔）:"));
		errorSet.add(wrongDate);
		errorSet.add(new Label("丢失数据帧:"));
		...
		nowDate = new Label();
		sildWin = new Label();
		nowState.add(new Label("当前发送帧:"));
		nowState.add(nowDate);
		nowState.add(new Label("滑动窗口:"));
		...
		startSend = new Button("开始发送");
		stopSend = new Button("停止发送");
		quit = new Button("退出");
		padding = new Label();
